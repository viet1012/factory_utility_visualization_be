...
public interface FacTimeSeriesRepository extends JpaRepository<F2UtilityPara, Long> {
	@Query(value = """
			-- NON-KVH: max theo bucket (HOUR)
			SELECT
			    sc.fac           AS fac,
			    ch.cate          AS cate,
			...
			-- KVH: SUM theo bucket (HOUR) (gộp Fac_A/B/C), fac trả ra KVH
			SELECT
			    'KVH'            AS fac,
			    ch.cate          AS cate,
			    ch.scada_id      AS scadaId,
			...
			JOIN F2_Utility_Scada sc
			  ON ch.scada_id = sc.scada_id
			WHERE :fac = 'KVH'
			  AND sc.fac IN ('Fac_A','Fac_B','Fac_C')
			  AND hi.pick_at >= :from
			  AND hi.pick_at <  :to
			  AND (:boxDeviceId IS NULL OR :boxDeviceId = '' OR hi.box_device_id = :boxDeviceId)
			  AND (:plcAddress IS NULL OR :plcAddress = '' OR hi.plc_address = :plcAddress)
			...
			ORDER BY cate, boxDeviceId, plcAddress, ts
			""", nativeQuery = true)
	List<FacTimeSeriesRow> getSeriesHourByFac(
	...
	@Query(value = """
			...
			-- KVH: SUM theo bucket (gộp Fac_A/B/C), fac trả ra KVH
			SELECT
			    'KVH'            AS fac,
			    ch.cate          AS cate,
			    ch.scada_id      AS scadaId,
			...
			ORDER BY cate, boxDeviceId, plcAddress, ts
			""", nativeQuery = true)
	List<FacTimeSeriesRow> getSeriesDayByFac(
			@Param("fac") String fac,
			@Param("from") LocalDateTime from,
			@Param("to") LocalDateTime to,
			@Param("boxDeviceId") String boxDeviceId,
