...
public interface UtilityMonthlyRepo extends JpaRepository<DummyEntity, Long> {
	...
	// KVH - NATIVE QUERY
	//
	// Không còn phụ thuộc dbo.sp_Utility_Monthly_Kvh.
	//
	// BUSINESS GIỮ NGUYÊN:
	// - Electricity = gross meter - Solar.
	// - Electricity cost = gross tariff cost - Solar tariff cost.
	// - Electricity chỉ dùng History_Main rows có MTD = 'MTD'.
	// - Solar chỉ dùng History_Main rows có MTD = 'MTD'.
	// - Water / Pipeline / Air dùng AVG/MIN/MAX.
	// - CURRENT và PREV dùng đúng 2 range, không load gap rows.
	// =========================================================
	@Query(
			value = """
          WITH ParaDedup AS (
          ...
          TargetPara AS (
              ...
              WHERE EXISTS (
                  ...
                  WHERE ch.box_device_id = pa.box_device_id
                    AND UPPER(LTRIM(RTRIM(ISNULL(ch.box_id, '')))) <> 'SOLAR'
              )
          ),
          ...
          Base AS (
              SELECT
                  ...
                  tp.unit,
                  hi.pick_at,
                  CAST(hi.[value] AS DECIMAL(19,6)) AS [value],
                  DATEPART(HOUR, hi.pick_at) AS HourNumber,
                  CASE
                      WHEN DATEPART(WEEKDAY, CAST(hi.pick_at AS DATE)) = 1
                          THEN '1'
                      ELSE '2-7'
                  END AS WD
              FROM TargetPara tp
          ...
          SolarBase AS (
              SELECT
                  'CURRENT' AS period_type,
                  hm.pick_at,
                  CAST(hm.[value] AS DECIMAL(19,6)) AS solar_value,
                  DATEPART(HOUR, hm.pick_at) AS HourNumber,
                  CASE
                      WHEN DATEPART(WEEKDAY, CAST(hm.pick_at AS DATE)) = 1
                          THEN '1'
                      ELSE '2-7'
                  END AS WD
              FROM SolarTargetPara stp
          ...
          SolarHourly AS (
              SELECT
                  ...
                  WD,
                  HourNumber,
                  SUM(solar_value) AS solar_hour_value
              FROM SolarBase
              GROUP BY
                  period_type,
                  WD,
          ...
          SolarMonthly AS (
              SELECT
                  CAST(
                      COALESCE(
                          SUM(CASE WHEN period_type = 'CURRENT'
                                   THEN solar_value ELSE 0 END),
                          0
                      )
                      AS DECIMAL(18,2)
                  ) AS solarValue,
                  ...
                  CAST(
                      COALESCE(
                          SUM(CASE WHEN period_type = 'PREV'
                                   THEN solar_value ELSE 0 END),
                          0
                      )
                      AS DECIMAL(18,2)
          ...
          EnergyMonthly AS (
              SELECT
                  ...
                  SUM(
                      CASE WHEN e.period_type = 'CURRENT'
                           THEN e.hour_value ELSE 0 END
                  ) AS gross_value,

                  SUM(
                      CASE WHEN e.period_type = 'PREV'
                           THEN e.hour_value ELSE 0 END
                  ) AS gross_prev_value,
                  ...
                  SUM(
                      CASE WHEN e.period_type = 'CURRENT'
                           THEN COALESCE(sh.solar_hour_value, 0) * r.vnd_rate
                           ELSE 0 END
                  ) AS solar_vndCost,
              ...
              SELECT
                  ...
                  CAST(
                      CASE
                          ...
                          THEN COALESCE(e.gross_prev_value, 0)
                               - COALESCE(s.prevSolarValue, 0)
                          ELSE 0
                      END
                      AS DECIMAL(18,2)
                  ...
                  CAST(
                      CASE
                          ...
                          THEN COALESCE(e.gross_vndCost, 0)
                               - COALESCE(e.solar_vndCost, 0)
                          ELSE 0
                      END
                      AS DECIMAL(18,2)
                  ...
                  CAST(
                      CASE
                          ...
                          THEN COALESCE(e.gross_prevVndCost, 0)
                               - COALESCE(e.prev_solar_vndCost, 0)
                          ELSE 0
                      END
                      AS DECIMAL(18,2)
              ...
              SELECT
                  ...
                  CAST(ROUND(AVG(
                      CASE WHEN period_type = 'CURRENT'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS avgValue,

                  CAST(ROUND(MIN(
                      CASE WHEN period_type = 'CURRENT'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS minValue,

                  CAST(ROUND(MAX(
                      CASE WHEN period_type = 'CURRENT'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS maxValue,

                  CAST(ROUND(AVG(
                      CASE WHEN period_type = 'PREV'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS prevAvgValue,

                  CAST(ROUND(MIN(
                      CASE WHEN period_type = 'PREV'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS prevMinValue,

                  CAST(ROUND(MAX(
                      CASE WHEN period_type = 'PREV'
                           THEN [value] END
                  ), 1) AS DECIMAL(18,1)) AS prevMaxValue
              FROM Base
          ...
          FinalRows AS (
              SELECT
                  ...
                  CAST(
                      e.vndCost / NULLIF(:exchange, 0) * :sepzone
                      AS DECIMAL(18,2)
                  ) AS usdCost,
                  ...
                  CAST(
                      e.prevVndCost / NULLIF(:exchange, 0) * :sepzone
                      AS DECIMAL(18,2)
                  ) AS prevUsdCost
              FROM EnergyNetMonthly e
          ...
          SELECT
              ...
              CAST(
                  CASE
                      WHEN COALESCE(f.prevValue, f.prevAvgValue, 0) = 0
                          THEN NULL
                      ELSE (
                          (
                              COALESCE(f.value, f.avgValue, 0)
